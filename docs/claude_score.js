...
//   computeScoreAverages(memes)     -> { text:number|null, image:number|null, n:int }
...
export function createClaudeScoreBadges(score) {
  ...
  if (reasoning) {
    ...
    pairs.title = `Текст ${t ?? "?"} / Картинка ${i ?? "?"}`;
  }
...
// actually carry numeric scores. `text` / `image` are null if no data.
export function computeScoreAverages(memes) {
  let tSum = 0, tN = 0, iSum = 0, iN = 0, any = 0;
...
// Fill a header slot with "Ср. оценка: X.X / Y.Y" (or dash if no data).
export function renderScoreAveragesInto(hostEl, avg) {
  if (!hostEl) return;
  hostEl.textContent = "";
  const fmt = v => v === null || v === undefined ? "\u2014" : v.toFixed(1);
  const label = el("span", "avg-score-label", "Ср. оценка:");
  const val = el("span", "avg-score-val", `${fmt(avg.text)} / ${fmt(avg.image)}`);
